import { act } from '@testing-library/react'
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest'
import { createTestKit } from '~/test-utils/factories/kit'
import { renderHookWithProviders } from '~/test-utils/react'

vi.mock('@tanstack/react-router', () => ({
  useNavigate: () => vi.fn(),
}))

vi.mock('~/features/kit-stock-add', () => ({
  addKitEvent: vi.fn(),
}))

import { addKitEvent } from '~/features/kit-stock-add'
import { useKitAdd } from './useKitAdd'

const addKitEventMock = vi.mocked(addKitEvent)

describe('useKitAdd', () => {
  beforeEach(() => {
    addKitEventMock.mockResolvedValue({
      id: 'event-1',
      userId: 'u-1',
      kitId: 'kit-1',
      delta: 1,
      reason: 'purchase',
      projectId: null,
      purchasedAt: null,
      priceYen: null,
      purchaseLocation: null,
      note: null,
      createdAt: new Date(),
    })
  })

  afterEach(() => {
    addKitEventMock.mockClear()
  })

  it('初期 state: phase=search', () => {
    const { result } = renderHookWithProviders(() => useKitAdd())
    expect(result.current.phase.kind).toBe('search')
  })

  it('selectKit で phase=add-stock に遷移', () => {
    const { result } = renderHookWithProviders(() => useKitAdd())
    const kit = createTestKit({ id: 'kit-1' })
    act(() => {
      result.current.selectKit(kit)
    })
    expect(result.current.phase.kind).toBe('add-stock')
    if (result.current.phase.kind === 'add-stock') {
      expect(result.current.phase.kit.id).toBe('kit-1')
    }
  })

  it('goToSearch で phase=search に戻る', () => {
    const { result } = renderHookWithProviders(() => useKitAdd())
    const kit = createTestKit({ id: 'kit-1' })
    act(() => {
      result.current.selectKit(kit)
    })
    act(() => {
      result.current.goToSearch()
    })
    expect(result.current.phase.kind).toBe('search')
  })

  it('handleStockSubmit 呼び出しで addKitEvent が期待引数で呼ばれる', async () => {
    const { result } = renderHookWithProviders(() => useKitAdd())

    await act(async () => {
      await result.current.handleStockSubmit('kit-1', {
        purchasedAt: '2026-05-01',
        purchasePriceYen: 1500,
        purchaseLocation: 'ヨドバシ',
        note: 'メモ',
      })
    })

    expect(addKitEventMock).toHaveBeenCalledOnce()
    expect(addKitEventMock).toHaveBeenCalledWith({
      data: expect.objectContaining({
        kitId: 'kit-1',
        delta: 1,
        reason: 'purchase',
        purchasedAt: '2026-05-01',
        priceYen: 1500,
        purchaseLocation: 'ヨドバシ',
        note: 'メモ',
      }),
    })
  })
})
