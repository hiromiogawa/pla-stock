...
interface ProjectCardProps {
  project: Project
  /** 紐付きキット名 (kit master 経由で resolve 済み)。null なら未紐付き。 */
  linkedKitName: string | null
  /** 紐付きキットの box art URL。サムネイルとして使う、null なら placeholder 表示。 */
...
export function ProjectCard({ project, linkedKitName, linkedKitBoxArtUrl }: ProjectCardProps) {
  return (
    <Link
      to="/projects/$id"
    ...
    >
      <div className="flex gap-3 items-start">
        <div className="w-16 h-16 shrink-0 rounded border border-border bg-muted/40 overflow-hidden flex items-center justify-center text-[10px] text-muted-foreground">
          {linkedKitBoxArtUrl ? (
            <img
