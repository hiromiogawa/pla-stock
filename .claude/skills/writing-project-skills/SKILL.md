...
metadata:                        # check-harness / 連鎖関係の機械検証用
  kind: orchestrator
  subskills: [skill-name, ...]
  subagents: [agent-name, ...]   # subagent を使う場合のみ
  trigger: <user 起動の状況>
---
```
...
- **skill のまま (`.claude/skills/`) 残す**: AI が状況判断で自動起動すべき判定系 (`testing` / `failure-record` / `adr` 等の handbook)。`kind: atomic` を frontmatter で明示
- **subagent (`.claude/agents/`) にする**: 独立 context で実行する必要があるもの (例: self-review の「新鮮な目」、rule-measure / rule-explore の独立計測)。Agent tool で dispatch
- **transitional**: `project-bootstrap` は `.claude/skills/` に `kind: orchestrator` 残置 (subagent 化判断は #188、check-harness で `TRANSITIONAL_ORCHESTRATOR_SKILLS` 例外)

### 設計上の注意

- 本物 slash command の引数は body 中で `$ARGUMENTS` プレースホルダで参照可能 (Claude Code が user 入力を substitute)
- 連鎖 (orchestrator) の subskills は metadata で機械検証 (check-harness が `.claude/skills/` + `.claude/commands/` 双方を探索して実在確認)
- AI 視点では「ユーザーに `/name` を依頼する」運用 (auto-trigger 構造的に不可なので毎回 user 入力が必要)
- skill / command 一覧は Claude Code が session 開始時 system-reminder で context 注入する。CLAUDE.md トリガー表生成は #227 で廃止 (description が SSoT、俯瞰は #229 drawio)
