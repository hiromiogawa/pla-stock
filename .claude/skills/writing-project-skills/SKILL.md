...
## slash command 化 (AI auto-trigger 無効化)

skill が AI auto-trigger では起動安定性が悪い (例: orchestrator 系、明示起動の儀式が本質のもの) 場合、frontmatter top-level に `disable-model-invocation: true` を追加することで:

- AI の skill リストから消える (auto-trigger 不可、Skill tool 起動も不可)
- ユーザーが `/<skill-name>` を入力したときに AI の context に skill 内容が読み込まれる (ユーザー明示起動のみ)

### 使用判断

- **Slash 化を採用する skill**: 明示起動の儀式が本質な orchestrator 全般。pla-stock では `dev-start` / `dev-complete` / `post-review` / `rule-cycle` / `design-decision` の 5 つに `disable-model-invocation: true` 付与済。`project-bootstrap` は別途扱い (Write tool 必要 + 新規プロジェクトでのみ動作、subagent 化候補で #188 で起票)
- **Slash 化しない skill**: AI が状況判断で自動起動すべき判定系 (`testing` / `failure-record` / `adr` 等の handbook)
- **subagent との使い分け**: subagent は独立 context で実行 (例: self-review の「新鮮な目」)、slash は親 context で実行する明示起動の skill

### 設計上の注意

- slash 化した skill は CLAUDE.md トリガー表で「ユーザーが /name で起動するとき」と記述する (auto-generated)
- AI 視点では「ユーザーに `/name` を依頼する」運用に変わるため、CLAUDE.md AI 運用ルール側でも「依頼」表現に統一
- skill 連鎖 (orchestrator) を slash 化する場合、配下 sub-skill / subagent との連携は AI 内部で行う (slash 入力 → skill 内容読込 → 連鎖 dispatch)
