...
metadata:
  kind: atomic
  trigger: ドキュメントを追加・更新するとき、生成物と手動ドキュメントの役割分担に迷ったとき、または CI / pre-push hook の docs 差分チェックが失敗したとき
---
...
|---|---|---|
| `CLAUDE.md` の `<!-- gen:skill-index -->` トリガー表 | `pnpm gen:skill-index` | `pnpm check:harness` (pre-commit hook の `check:parallel` 経由で機械強制) |
| `docs/adr/README.md` | `pnpm gen:adr-index` | `pnpm gen:adr-index --check` (pre-push hook で機械強制、#178) |

drift 検出が落ちた場合は対応するスクリプトを実行してコミット (`pnpm gen:skill-index` / `pnpm gen:adr-index`)。

> NOTE: 将来 typedoc / dependency-graph の自動生成を追加する場合は `docs/generated/` ディレクトリを切り、同じ drift 検出パターンを踏襲する。
...
- [ ] skill / agent / ADR を追加・編集した場合、`pnpm gen:skill-index` / `pnpm gen:adr-index` の再生成が必要か (pre-commit / pre-push で drift 検出されるが、事前実行で hook reject を防げる)
