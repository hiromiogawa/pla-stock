...
指摘に対応し、**REQUIRED SUB-COMMAND:** `/dev-complete` の Step 1-3（self-review subagent dispatch → docs-freshness → conventional-commits）を再実行する。Step 1 の self-review は Agent tool で subagent dispatch される (詳細は `.claude/commands/dev-complete.md` / `.claude/agents/self-review.md`)。
