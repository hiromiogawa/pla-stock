...
| skill / command | 作業の型 (orchestration only、規約本文は docs/rules/ 参照) | project orchestrator 5 (commands) + atomic 15 (skills、dispatch reference 含む) + subagent 4。frontmatter は Claude Code 公式形式のみ。#111 で sdd 廃止 + 4 skill を docs/rules/ 参照型に rewrite | 俯瞰図は #229 drawio で別途 |
...
- **atomic skill**（`.claude/skills/`, 15, frontmatter 管理）: 本リポジトリ固有。
  ...
  **orchestration only 原則** (ADR-0018): 規約本文は `docs/rules/` 参照、
  ...
  - `adr` `code-quality` `conventional-commits` `docs-freshness`
    ...
    `writing-project-skills` (sdd は #111 で未使用判定により廃止)
